...
function savePost(){
  ...
  firebase.database().ref('post').limitToLast(1)
...
firebase.database().ref('post')
 ...
 .on('child_added', (newPost) => {
   ...
   <div class = "new_post_style col-xs-8 col-sm-12 col-md-8 col-lg-8">           
   <textarea class="input_text_post"> ${newPost.val().texto} </textarea>
   <div id="contenedor-sub-post" class="row">
   <img id="userPicture_imprimir" src="img/usuario.png"> 
...
function eliminarPost(currentPostKey) {
  let confirmar = confirm('¿Seguro que deseas borrar esta publicación');
  if (confirmar == true ) {
  firebase.database().ref('post').child(currentPostKey).remove()
...
const showEditPost = () => {
  ...
  showPostArea.innerHTML += `
  <div class = "new_post_style col-xs-8 col-sm-12 col-md-8 col-lg-8">           
  <textarea class="input_text_post"> ${currentPostKey} </textarea>
  <div id="contenedor-sub-post" class="row">
  <img id="userPicture_imprimir" src="src/assets/img/usuario.png"> 
  <div> ${newPost.val().nombre}</div> 
  <button id="btnLikes" class = "btn-post" onclick="sumarLikes('${currentPostKey}')"><i class="fas fa-heart" ><p id="showLikes"></p></i></button>
...
// Función contador de LIKES
let contador = 0;
function sumarLikes(currentPostKey) {
  console.log("imprime")
  contador = contador + 1;
...
function actualizarLike(currentPostKey) {
